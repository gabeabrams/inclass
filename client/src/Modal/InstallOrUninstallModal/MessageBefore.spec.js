...
describe('client > src > Modal > InstallOrUninstallModal > MessageBefore', () => {
  ...
  it('Displays the correct title for uninstall', () => {
    ...
    assert(driver.elementExists('modal-title'), 'modal title is not rendered');
    assert.equal(driver.getText('modal-title'), 'Please Read Before Uninstallling', 'did not display correct title when uninstlling');
  });
});
